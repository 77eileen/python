...
-- 구매정보와 상품 분류를 연결
-- 전자 대신에 전자제품처럼 보기 좋게 코드마스터에서 조인해서 출력
-- buytbl, code_master
...
cm.code_name_ as code_name_mob1,
cmt.code_name
from usertbl u
left join buytbl b
...
-- GetUserList 프로시져 사용 후
call GetUserList() ;
-- GetUserByAddr 프로시져 생성 후
call GetUserByAddr('경남');
call GetUserByAddr('');
call GetUserByAddr(null);
-- call GetUserByAddr(); (x) 매개변수는 반드시 값을 줘야함 


-- ManageCode 프로시져 생성 후
call ManageCode('ADDR', '대전', '대전광역시', 7, 'Y', 'delete');
-- invalid action! update or insert
-- ADDR에 부산코드가 없음
